...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	for x := .0; x < float64(dstBds.Dx()); x += float64(srcBds.Dx()) * g.scale {
		for y := .0; y < float64(dstBds.Dy()); y += float64(srcBds.Dy()) * g.scale {
	...
	debug.DrawPrintf(screen, debug.BottomLeft, "Repetitions %d", n)
}
